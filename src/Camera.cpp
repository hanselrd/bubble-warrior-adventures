...
void Camera::update(float delta) {
    ...
    if (playerPos.x < (windowSize.x / 2.0f) * CAMERA_ZOOM_FACTOR)
        _view.setCenter((windowSize.x / 2.0f) * CAMERA_ZOOM_FACTOR, _view.getCenter().y);

    if (playerPos.y < (windowSize.y / 2.0f) * CAMERA_ZOOM_FACTOR)
        _view.setCenter(_view.getCenter().x, (windowSize.y / 2.0f) * CAMERA_ZOOM_FACTOR);

    if (playerPos.x > _map->getSize().x - ((windowSize.x / 2.0f) * CAMERA_ZOOM_FACTOR))
        _view.setCenter(_map->getSize().x - ((windowSize.x / 2.0f) * CAMERA_ZOOM_FACTOR), _view.getCenter().y);

    if (playerPos.y > _map->getSize().y - ((windowSize.y / 2.0f) * CAMERA_ZOOM_FACTOR))
